...
def process_image(path):
    ...
    if temp_image is None or trgt_image.size is None:
        return
...
def split_set():
    if os.path.isdir(config.train_data_path):
        shutil.rmtree(config.train_data_path)
    if os.path.isdir(config.test_data_path):
...
def data_augmentation():
    ...
    for dir in tqdm(listdir(config.data_path)):
        for image in listdir(config.data_path+'/'+dir):
            ...
            data_image_path = config.data_path + path + '.jpg'
            data_image=cv2.imread(data_image_path)
            data_image=da.snv(data_image)
            Path(config.data_da_path + path.split('/')[0]).mkdir(parents=True, exist_ok=True)
            cv2.imwrite(config.data_da_path + path + '.jpg', data_image)
    return
